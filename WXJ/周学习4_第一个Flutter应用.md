#第一个Flutter应用：计数器
##创建Flutter应用模版
+ 创建新的Flutter工程后可以得到一个计数器应用的Demo，应用源码如下：
...
import 'package:flutter/material.dart';
```
此代码作用是导入Material UI组件库。Material是一种标准的移动端和web端的视觉设计语言， Flutter默认提供了一套丰富的Material风格的UI组件。
...
###State类
1. 计数器示例需要维护一个点击次数计数器，所以定义一个_counter状态：  
```
int _counter = 0;
```
2. 设置状态的自增函数：  
